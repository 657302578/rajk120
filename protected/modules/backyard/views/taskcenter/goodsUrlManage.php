...
<body>
	...
	<div class="container-fluid content">
		...
		<div class="main">
            <div class="row">
            	<div class="col-lg-12">
            		<h3 class="page-header"><i class="fa fa-laptop"></i> 平台任务</h3>
            		<ol class="breadcrumb">
            			<li><i class="fa fa-home"></i><a href="<?php  echo $this->createUrl('default/index');?>">首页</a></li>
            			<li><i class="fa fa-laptop"></i>商品链接列表</li>					  	
            		</ol>
            	</div>
            </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <?php
                            ...
                            foreach($proInfo as $item){
                                $publishInfo=User::model()->findByPk($item->uid);//发布人信息
                        ?>
                            <tr>
                                <td>
									<?php echo $item->id;?>
								...
								<td>
								<?php 
									$shopInfo = Blindwangwang::model()->findByPk($item->shop_id);
								...
								<td>
									<?php
										...
										{
									?>
										<a href="javascript:checkGoodsUrl(<?php echo $item->id;?>,1)" title="审核通过">审核通过</a>
									<?php
										}else{
									?>
										<a href="javascript:checkGoodsUrl(<?php echo $item->id;?>,0)" title="审核通过">审核不通过</a>
									<?php
										}
									?>
								</td>
...
<script>
function checkGoodsUrl(id,s)
{
	$.ajax({
		type:"POST",
		url:"<?php echo $this->createUrl('taskcenter/checkGoodsUrl');?>",
		data:"id="+id+"&s="+s,
